...
/* Heuristic minimum address for a valid heap pointer.  Pointers below this
 * threshold (NULL, small integers, error/enum values mistakenly used as
 * pointers) are treated as invalid by the bridge guards.  This is a sanity
 * check, not a security boundary. */
static const uintptr_t kMGLMinValidPointer = 0x1000;

/* MGLMetalBridgeTarget protocol is now declared in mgl_metal_bridge.h so
 * MGLRenderer can formally conform to it and the compiler verifies all
