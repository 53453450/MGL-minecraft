            ...
            if (isFragment) {
                {
                    ...
                    int suppress_missing_raw = suppressMissing ? 1 : 0;
                    int used_fallback_raw = usedFallback ? 1 : 0;
                    bool recovered = mglSampledRecoverFragmentDepthTexture(
                        (__bridge void *)self, &ptr, &texture_raw, sampledName,
                        spirvBinding, textureUnit, expectedType,
                        (uint32_t)expectedKind, fragmentProgramName,
                        &suppress_missing_raw, &used_fallback_raw);
                    texture = (__bridge id)texture_raw;
                    suppressMissing = suppress_missing_raw ? YES : NO;
