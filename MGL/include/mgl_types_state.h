 ...
 *
 * GLMState is 92KB (measured 91,952 B on arm64), but only ~61KB of fields
 * (measured 61,240 B) are read by the Metal encoder during batch replay.
 * The cold regions are:
 *   - 11 embedded HashTables (sync_table + 10 others, ~1KB): restored from savedState at replay time
 *     (snapshot copies are stale — internal keys/states arrays may have been
 *     reallocated)
 *   - 11 of 16 buffer_base types (29.7KB): never read by the encoder during
 ...
 * ~33.4% (30.7KB) per snapshot creation and per restore.
 *
...
/* Actual bytes mglCopyHotStateFields writes per snapshot: the whole struct
 * minus the two skipped cold regions.  The same regions are pinned by the
 * _Static_asserts below, so this stays honest as the struct evolves. */
static inline size_t mglSnapshotHotStateBytes(void)
{
    return sizeof(GLMState)
         /* 11 embedded HashTables (sync_table + 10 others) */
         - 11 * sizeof(HashTable)
         /* the cold buffer_base types, counted from the X-macro list */
