...
typedef struct MGLMetalCallbackCensus_t {
    uint32_t total;
    uint32_t non_legacy;
    uint32_t legacy;
    uint32_t null_entries;
} MGLMetalCallbackCensus;

/* Inspect the actual GLMMetalFuncs table.  A callback is legacy only when its
 * installed pointer is the selector-forwarding function generated by the
 * matching MGL_MTL_FUNC_LIST entry. */
