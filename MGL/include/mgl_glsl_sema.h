...
typedef struct MGLIRSymbol {
    ...
    /* 1 = declared as `uniform|buffer Block { ... } [inst]` (interface
     * block), not `struct S { ... }; uniform S s;` (default-block struct). */
    int is_interface_block;
    /* Image layout format string (static, from parser); NULL if N/A. */
