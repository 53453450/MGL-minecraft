...
> Snapshot: `main` @ C1d air math builtins (~15.2k air / ~20.5k render LOC). Re-measure with `wc -l` after splits.
...
|----|-----:|------|----------------------|
| `MGL/src/mgl_air_backend.cpp` | ~15216 | GLSL AST → LLVM AIR → `.metallib` | Mixes expr/stmt emit, stage ABI, legacy rewrite, reflect helpers; **type (C1b) + resource (C1c) + math builtins (C1d) extracted** |
...
|-------------:|-----------------|
| ~87–170 | GS AST→ABI map; C1b/C1c/C1d facades; `storeStageOut` |
...
| Residual in monolith | emitExpr / matrix / stmt / module-assembly VarSym / legacy |
...
**Next strip suggestion:** module-assembly VarSym classify/location, or later expr facade — still defer emitExpr/matrix bodies. Do **not** sink back into `mgl_air_backend.cpp`. Trajectory under ~15k air TU.
...
- [ ] Future knives: continue by domain table (module-assembly VarSym / later expr facade, or binding-policy residual); keep golden before large moves (ARCH); do not re-enable CI until Paravirt sorted
