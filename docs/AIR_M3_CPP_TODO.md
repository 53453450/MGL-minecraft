...
> **C1 note:** IntegerReadback Convert/Source/Packed/Classify **and** Y-flip (`CopyRows`) / depth pack / `GetTexImagePlan` / `MSAAArrayLayerStride` now live in `mgl_readback_policy.{h,c}` (included via `mgl_render.h`). Metal `EncodeMultisampleResolve*` residual stays in `mgl_render.cpp`. Do not re-sink into `mgl_render.cpp`.
