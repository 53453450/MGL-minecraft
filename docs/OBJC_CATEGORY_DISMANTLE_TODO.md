...
| `+Buffer.m` | ~1342 | 中 | map/CoW/shadow plan → C++（O5.1：vertex-index + dirty-buffer 决策已沉 C 函数；**vertex-attrib buffer map 已整段沉 `mgl_vertex_attrib_plan.*` + harness**）；ObjC 只 MTLBuffer 物化与逐 attribute resolve |
