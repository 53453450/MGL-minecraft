...
20. **下一刀**：残量 `dyn`(~366) / `flush`(~380) / `trace`(~374) / `issue`(~200)；C1 已开但勿掩盖 Batch 残量；**C1b done**=air type helpers→`mgl_air_type.*` + non-Metal `test_mgl_air_type` golden；**C1c done**=air resource collection→`mgl_air_resource.*`；O4.1 Metal resolve encode + flip-aware format convert 可后迁；继续 ensure/resolve/submit 压薄；O3 / O6 可并行
